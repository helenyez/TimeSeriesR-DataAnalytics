...
references: Ryan, J. (2019). Manipulating Time Series Data in R with xts and zoo. Retrieved from  https://www.datacamp.com/courses/manipulating-time-series-data-in-r-with-xts-zoo
---
...
Extract a range of dates using the ISO-8601 feature of xts. The create subset of the new object with specific start and end dates using the same notation.
...
To view only a subset of time for each day over multiple days using xts, we can slice days easily by using special notation in the i = argument to the single bracket extraction (i.e. [i, j]).
...
```{r}
# Extract recurring morning hours between 8AM and 10AM from irreg ts
...
To locate data by relative time, you describe a relative position in time. A simple example would be something like the last 3 weeks of a series, or the first day of current month.

xts has the necessary prerequisites built in: first() and last() functions 

Use case: Extract relative observations from a data set called temps, a time series of summer temperature data from Chicago, IL, USA.
...
xts objects respect time. By design when we perform any binary operation using two xts objects, these objects are first aligned using the intersection of the indexes.
...
re-indexing the data before an operation makes it possible to preserve the dimensions of the data by leveraging the same mechanism that xts uses internally in its own Ops method (the code dispatched when you call + or similar).

The third way involves modifying the two series we want by assuring you have some union of dates - the dates you require in your final output. Use `merge(b, index(a))`
